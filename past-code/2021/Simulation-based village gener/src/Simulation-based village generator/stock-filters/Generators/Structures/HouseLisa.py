...
def generateHouse(matrix, wood_material, h_min, h_max, x_min, x_max, z_min, z_max, orientation = "O", door_suggestion = None, ceiling = None, job=None):
	...
	# Lisa addition, original was: house.orientation = getOrientation(matrix, house.lotArea)
	if orientation == "O":
	...
	generateInterior(matrix, h_min, ceiling_bottom, house.buildArea.x_min, house.buildArea.x_max, house.buildArea.z_min, house.buildArea.z_max, ceiling, job)
	generateGarden(logger, matrix, house, fence, fence_gate_ID)

	return house.entranceLot #(door_x, door_z)#house
...
def generateInterior(matrix, h_min, h_max, x_min, x_max, z_min, z_max, wood, job):

	def generateJobObject(matrix, y, x, z, job):
		job_dict = {"cleric": 117, "crafter": 58, "smith": 61, "enchanter": 116, "blacksmith": 145}
		object = job_dict[job]
		matrix.setValue(y+1, x-1, z+1, object)

	generateCarpet(matrix.matrix, h_min + 1, x_min + 1, x_max, z_min + 1, z_max)
	if job:
		generateJobObject(matrix, h_min, x_max, z_min, job)
	else:
